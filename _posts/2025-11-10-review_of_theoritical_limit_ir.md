...
I recently came across [On the Theoretical Limitations of Embedding-Based Retrieval](https://arxiv.org/pdf/2508.21038), a paper which overlaps with my interests about
...
I enjoy the type of work in this paper and wish to see more of it, but I'm going to take a more critical look. Papers in this vein need to be rigorous
...
While there is overlap, the video has significantly more background on IR (which the reader is assumed to have) and doesn't dive as deeply into the methodological issues
of the paper. Moreover, this blog will contain the results of additional experiments and suggestions for how this 
work could be used. I recommend watching the video first, especially for people unfamiliar with IR or who are newer to field.
It discusses the more classic framing or IR that has been neglected in more recent RAG work.

Now on to the show. The blog will have the following structure. First discuss the papers 3 contributions, followed by 
...
$C$ is the corpus of documents $c$. $c$ is used to avoid confusion with dimension $d$
...
$$sign_rank(rel)-1 <= rank_rop(rel) = rank_rt rel <= rank_gt rel < sign_rank(rel)$$
...
So the authors pitch this dataset as an easy dataset BM-25 can do, but neural models can't.
There are three design choices I find questionable:
1. BM-25 is used to filter for profiles that are likely to be false positives
2. Removal of hypernyms in the attributes benefits models that don't capture the semantics of words like the baseline (bm-25)
...
IMO these choices unfairly penalize single embedding models and fail to reflect what real IR tasks look like. For example
the colbert results are quite a bit better than single embeddings, but still quite a bit lower than BM-25.
Given how Colbert is said to transfer domains better <!-- TODO: Include citation -->
I think difference can be accounted for by that alone (see point 3.)
...
don't go far enough, but they are a step in the right direction IMO.


### Estimates of min dims over real datasets

For BEIR, calculate dims using free embedding across different datasets.

Show values for datasets 

### Variabilitiy of the method over multiple runs

Adam is none to be effected by initial conditions, what happens 
if we have a different starting matrix or seed. What do we
do then?

### Effects of Changes in $rel$

This begs some questions about how changes in $rel$ effect the dimensions required. These
...
The following subsections will describe experiments to show the practical implications of each of these 

#### Effects of p(y|x)

Compute a table across different beir tasks and show how variable the rank computed  is across the tasks.
Is it consistent?
Cross k-fold over train splits of beir tasks. See how variable dimension is to small changes.
...
#### Effects of different splits

Experiment with combining train and test rel matrices and seeing how dramatically 
the $d$ computed.

Similar to experiment above, but more finite in scope. 
...
### Alternative Solutions to Single Embeddings
...
to subdivide a corpus into different areas or have multiple representations for a single document that are used 
under different circumstances. It is worth noting at that point, other representations might make more 
sense and it would heavily depend on the specific problem.

### Extending the Finding to Reranking

Lets accept the premise that single embeddings have been shown to have (a yet to be defined) limit 
on the number of documents they can capture the top-k for.
What if instead of using them to get the top-k directly, I just used them as rerankers?
...
for multi-vector methods like plaid <!-- todo: verify and add a citation -->. A theory that gives us 
bounds on k' would be interesting.
