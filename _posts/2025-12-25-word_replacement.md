...
An argument could be made that implicitly this happens since the model is likely going to favor web like words and phases in the replacement models pre-training corpus (being a language model), but I have no proof for this.
...
but GPT-4o has a much larger corpus and likely a different distribution.
...
performance. In other words, do replacements which make a given item more or less similar to the pre-training distribution improve performance?
...
Framing the results as binary is naive, but would allow for use of a simple statistical test. The phi coefficient would work well for this.
...
measure of correlation between the document and the distributions and performance.
